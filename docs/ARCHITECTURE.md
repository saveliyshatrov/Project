...
Browser loads JS bundle
    ...
    │
    ▼
Widget controllers fetch data via resolvers
    │
    ├── GET /resolver?resolver=...
    ├── Server executes resolver, returns collections
    │
    ▼
...
- **Dev** — Custom Express server (`dev-server.mjs`) runs webpack in watch mode for both mobile and desktop, serves correct `index.html` based on User-Agent
- **Prod** — `webpack.prod.config.ts` builds mobile + desktop bundles to `client/dist/mobile/` and `client/dist/desktop/`
